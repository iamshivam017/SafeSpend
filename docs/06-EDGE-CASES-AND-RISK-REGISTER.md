...
| R11 | Recurrence mis-inferred (one-time spike projected, or recurring salary missed) | official R9; observed message_02 one-time adjustment | **High** | **High** | 25-sample regression + per-user audit | same user+category+direction periodicity; one-time flags from evidence; conservative amounts | T-recur | Open |
