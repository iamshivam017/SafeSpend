...
| R11 | Recurrence mis-inferred (one-time spike projected, or recurring salary missed) | official R9; observed message_02 one-time adjustment | **High** | **High** | 25-sample regression + per-user audit | deterministic gap-bucket detection with explicit params (D10/D18); majority rule tolerates one-off spikes; median-credit/max-debit amounts | T-recur (8 tests) | Mitigated (Phase 5 calibration pending) |
