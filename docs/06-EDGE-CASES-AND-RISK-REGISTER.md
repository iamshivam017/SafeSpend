...
| R11 | Recurrence mis-inferred (one-time spike projected, or recurring salary missed) | official R9; observed message_02 one-time adjustment | **High** | **High** | 25-sample regression + per-user audit | deterministic gap-bucket detection + day-of-month clustering + two-concept classification (D10 rev.3); monthly-commitments-only projection for variable streams; sample-plan audit + coverage audit (UNACCOUNTED=0) as oracles (D22/D23) | 14 recurrence/coverage tests + 2 audits | Mitigated (Phase 2.2: audit 0 contradictions, coverage 0 unaccounted; Phase 5 calibration pending) |
